...
function ents.PrintTableEnt_Pos(tbl)
	for k, v in pairs(tbl) do
		print(v:Pos())
	end
end

function ents.PrintTableEnt_OcSpawn(tbl)
	for k, v in pairs(tbl) do
		print("StartDisabled","=",v["StartDisabled"])
	end
end

if SERVER then 

	function ents.FindByName_Fire( name, _input, param, delay )
		--if not isstring( name ) then return end
		--if not isstring( _input ) then _input = "" end
		--if not isstring( param ) then param = "" end
		--if not isnumber( delay ) then delay = 0 end
		for k, v in pairs( ents.FindByName(name) ) do  
			v:Fire( _input, param, delay  ) 
		end
	end
end