...
if _DEBUG then
	...
	local hl2c_log = GetConVar("hl2c_log") or CreateConVar("hl2c_log", "none", FCVAR_REPLICATED + FCVAR_ARCHIVE + FCVAR_NOTIFY, "Enable logging groups seperated by comma")

	local function ProcessLogGroups()

		local val = hl2c_log:GetString()
	...
	function LogGroup(group, color, ...)
		...
		if SERVER then
			print(timestamp .. "[SV:" .. group .. "] " .. tostring(printResult))
		else
			if epoe and epoe.Print then
				epoe.Print(timestamp .. "[CL:" .. group .. "] " .. tostring(printResult))
			else
				print(timestamp .. "[CL:" .. group .. "] " .. tostring(printResult))
			end
		end
	...
	concommand.Add( "hl2c_log_groups", function( ply, cmd, args )
		ProcessLogGroups()
		local activeGroups = {}
