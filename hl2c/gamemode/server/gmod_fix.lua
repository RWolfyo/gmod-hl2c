local DbgPrint = GetLogging("FuncTank_Fix")

// Note that we must use that for Obsidian Conflict
local function ReplaceFuncTankVolume(ent, volname)

	local ent = ent
	local newName = volname
...
hook.Add("EntityKeyValue", "Hl2c_FuncTank", function(ent, key, val)
