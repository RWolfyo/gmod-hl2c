...
-- ****
-- This is an edit of the weapon_medkit that comes with Garry's Mod 13 so it works better with Co-Op. All credits to robotboy655 and MaxOfS2D.
-- ****

SWEP.PrintName			= "Medkit"
SWEP.Author			= "robotboy655 & MaxOfS2D"
SWEP.Purpose    	= "Heal people with your medkit."
...
SWEP.HealAmountFastTake = 5 -- Fast healing ammo taken
SWEP.HealAmountFastGive = 1 -- Fast healing health given
...
function SWEP:PrimaryAttack()
	...
	if ( IsValid( ent ) && self:Clip1() >= need && ( ent:IsPlayer() || ent:IsPet() ) && ent:Health() < ent:GetMaxHealth() ) then
		
		self:TakePrimaryAmmo( need )
		
		ent:SetHealth( math.min( ent:GetMaxHealth(), ent:Health() + need ) )
		self.Owner:SetNWBool("PROG_HEAL",true)
